...
let manifest = http get --headers [authorization $"Bearer ($token)"] $"https://index.docker.io/v2/($repository)/($image)/manifests/($tag)" | from json
...
match $manifest.mediaType {
    ...
    "application/vnd.oci.image.index.v1+json" => {
        ...
        let arch_manifest = http get --headers [authorization $"Bearer ($token)"] $"https://index.docker.io/v2/($repository)/($image)/manifests/($arch_digest)" | from json
        ...
        for layer in $layers {
            ...
            let url = $"https://index.docker.io/v2/($repository)/($image)/blobs/($digest)"
